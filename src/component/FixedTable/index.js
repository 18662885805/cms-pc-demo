...
import { Pagination, Checkbox, Icon, Select, Button, Input, message, DatePicker } from "antd";
...
class FixedTable extends Component {
  ...
  componentDidUpdate(prevProps) {
    const { dataSource: prevDataSource, reset: prevReset, refresh: prevRefresh } = prevProps;
    const { dataSource, reset, refresh } = this.props;
  ...
  expandRow = index => {
    ...
    const hasIndex = expandedRows.indexOf(index);
  ...
  handleExpandRow = ({ rowIndex, width, height }) => {
    const { expandedRows, processData } = this.state;
    const { expand, dataSource } = this.props;
    if (expandedRows.indexOf(rowIndex) < 0) {
      return null;
    }
    if (expand) {
      return expand(processData[rowIndex], this.getSubRowHeight(rowIndex));
    }
  }
  getSubRowHeight = index => {
    ...
    if (expandedRows.indexOf(index) > -1 && obj && Array.isArray(obj.efmExpands)) {
      if (obj.efmExpands.length > 0) {
        return 35 * obj.efmExpands.length >= 245 ? 245 : 35 * obj.efmExpands.length;
      } else {
        return 35;
      }
    } else {
  ...
  render () {
    const { columns, pagination, loading, expand, height, noCheckbox, noHeader, showArrow, showArrowWidth } = this.props;
    const { selectOption, filters, sorts, searchs, ranges, ops, rangesDate, expandedRows } = this.state;
              ...
              ({ width, height }) => {
                return (
                  <Table
                    ...
                    headerHeight={36}
                    rowHeight={35}
                    rowExpanded={this.handleExpandRow}
                    subRowHeightGetter={this.getSubRowHeight}
                    headerHeight={noHeader ? 0 : 36}
                  >
                    {
                      expand
                        ? <Column
                            ...
                            ({ rowIndex }) => {
                              return (
                                <div
                                  ...
                                  onClick={() => this.expandRow(rowIndex)}
                                >
                    ...
                    }
                    {
                      noCheckbox
                        ...
                        : <Column
                          ...
                          cell={
                            ({ rowIndex }) => {
                              const rowData = processData[rowIndex];
                          ...
                          <Column
                              ...
                              () => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div title={c.title} className={styles.headerItemContent}>{c.title}</div>
                                    {
                                      c.sorter || c.filter
                                        ? <div className={styles.headerItemIcon}>
                                          {
                                            c.sorter
                                              ? <div style={{width: 18, paddingTop: 3}}>
                                                <Icon
                                                  type="caret-up"
                            ...
                            cell={
                              ({ rowIndex }) => {
                                ...
                                return (
                                  <div className={styles.Cell}>
                                    {c.render(rowData[c.dataIndex], rowData, rowIndex)}
                                  </div>
                                );
                              }
                            }
