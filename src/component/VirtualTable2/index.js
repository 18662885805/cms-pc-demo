...
class VirtualTable extends Component {
  state = {
    ...
    scrollTop: this.props.defaultScrollTop || 0
  }
  componentDidMount() {
    document.onclick = e => {
  ...
  onScrollbarPresenceChange = () => {
    this.setState({ headerPaddingRight: 16});
  }
  onScroll = ({scrollTop}) => {
    if (scrollTop) {
  ...
  render () {
    const { pagination, noHeader } = this.props;
    let {
      processData,
      scrollTop
    } = this.state;
