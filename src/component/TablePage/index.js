...
class TablePage extends Component {
  ...
  getList = params => {
    ...
    const { getFn, dataMap, dataFilter,getMockData, param,noProjectId } = this.props;
    ...
    if(getMockData){
      pagination.total = getMockData&&getMockData.length;
      const firstData = getMockData[0];
      if (firstData && _util.getType(firstData.info) === "Object" && firstData.next) {
        getMockData = getMockData.map(r => {
          return {
            uid: r.id,
            ...r.info
          };
      ...
      if (typeof dataFilter === "function") {
        getMockData = dataFilter(getMockData);
      }

      this.setState({ data: getMockData, pagination, loading: false });
      return
    }
  ...
  doEnable = () => {
    ...
    this.props.enableFn({ ids: selectedRowKeys.join(","),operation:1, project_id: _util.getStorage('project_id') }).then(() => {
  ...
  doWithdraw = () => {
    ...
    const values = {
      ...
      project: _util.getStorage('project_id')
    };
    if (withdrawFirst) {
      values.is_first = 1;
    }
  ...
  doBack = () => {
    ...
    const values = {
      id: selectedRows.map(s => s.id).join(","),
      remarks: backRemark,
      project_id: _util.getStorage('project_id'),
      back_index: 1
    };
  ...
  doDisable = () => {
    ...
    this.props.disableFn({ ids: selectedRowKeys.join(","),operation:2, project_id: _util.getStorage('project_id') }).then(() => {
