...
class VirtualTable extends Component {
  state = {
    ...
    // sorts: _util.getSession('sort') ? {field: _util.getSession('c').dataIndex, type: _util.getSession('sortType')} : {},TODO2019
    sorts:{},
    searchs: {}, //搜索筛选，暂未用到
    ranges: {}, //数字范围筛选，
    rangesDate: {}, //日期筛选，
  ...
  componentWillReceiveProps(nextProps){
    // console.log('1803','componentWillReceiveProps')
    // if(nextProps.dataSource !== this.props.dataSource){
    // }
    // if(nextProps.loading !== this.props.loading){
    // }
    // if(nextProps.onPaginationChange !== this.props.onPaginationChange){
    // }
  ...
  resetConfig = (fn) => { //清除所有状态
    ...
    // _util.removeSession('filters');
    // _util.removeSession('filterIndex');
    // _util.removeSession('filterSelected')
  }
  componentDidUpdate(prevProps) {
    ...
    //TODO2019
    // if(_util.getSession('sort')){
    //   this.doSort(_util.getSession('c'),_util.getSession('sortType'))
    // }
  }
  ...
  updateData = (needPreData) => { //根据状态更新数据
    console.log('0401','updateData')
  ...
  doSort = (c, sortType) => { //排序
    _util.setSession("c",c);
    _util.setSession("sortType",sortType);
    const { sorts } = this.state;
    const { field, type } = sorts;
  ...
  rowRenderer = ({ index, key, style, parent }) => { //行渲染
    ...
    //console.log('0401',expandedRows,selectedRows,processData)
    return (
      <div style={{
        ...style
      }} key={key}>
        <div
          ...
          style={{
            ...
            lineHeight: "35px",
            height: 35,
            backgroundColor:selectedRows.indexOf(processData[index].id)>-1?"#f6f8fa":null
          }}>
          {
            expand
          &&
  ...
  render () {
    const { pagination, noHeader,appState,clearScrollTop,rememberPosition } = this.props;
