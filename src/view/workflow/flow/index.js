...
import {Popconfirm, Divider, message, Tag} from "antd";
import CommonUtil from "@utils/common";
import MyBreadcrumb from "@component/bread-crumb";
import { flow, flowDelete } from "@apis/workflow/flow";
...
export default class extends React.Component {
  ...
  onDeleteOne(id) {
    ...
    const { formatMessage } = this.props.intl;
    flowDelete(id, {project_id: _util.getStorage('project_id') }).then((res) => {
      if(res){
  ...
  render() {
    ...
    const bread = [
      ...
      {
        name: formatMessage({
          id: "page.component.workFlow.flow1",
      ...
      <div>
        ...
        <div className="content-wrapper">
          <TablePage
            ...
            getFn={flow}
            columns={column}
            addPath={canAdd&&"/workflow/flow/add"}
            excelName={'工作流类型'}
            dataMap={data => {
