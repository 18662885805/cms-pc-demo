...
import { privacyPost, privacyDetail, privacyPut } from "@apis/myadmin/privacy";
...
class PrivacyAdd extends React.Component {
  ...
  componentDidMount() {
    ...
    if (id) {
      privacyDetail(id).then(res => {
        const { results } = res.data;

        if (results) {
          const { p_type, title, content, content_en } = results;
    ...
    this.props.menuState.changeMenuCurrentUrl("/system/privacy");
  ...
  handleSubmit = () => {
    ...
    privacyPost(params).then(res => {
      message.success(formatMessage(messages.alarm8));
      this.props.history.goBack();
    });
  }
