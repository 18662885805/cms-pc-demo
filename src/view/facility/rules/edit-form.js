...
import {
  Form, Button, Modal, Input, Select, Switch,
  Spin, Icon, message, Upload, Checkbox, Cascader, TreeSelect, Row, Col, Radio, List
} from 'antd'
...
import {RulesPut, RulesDetail} from '@apis/facility/rules'
...
@injectIntl
class RulesEditForm extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  componentDidMount() {
    if (this.props.location.state === undefined || this.props.location.state.id === undefined) {
      ...
      RulesDetail(this.props.location.state.id).then((res) => {
        ...
        let source_list = []
        if (results.guide_menu) {
          results.guide_menu.split(',').map((value, index) => {
        ...
        let steps = res.data.results.step_data;
        const getValue = (obj) => {
          const tempObj = {}
          ...
          if (obj.content) {
            tempObj.content = []
            obj.content.map(o => {
              tempObj.content.push({name: o.name,is_fault: o.is_fault == 'True' ? true : false, has_remarks: o.has_remarks})
            })
          }
          return tempObj
        }
        ...
        this.setState({
          ...
          name: results.name,
          key_label: `${results.key_abbr}-${results.key_name}`,
          related_key: results.key_id,
          mtype: results.mtype,
          isCycle: results.is_cycle,
          interval: results.interval,
          unit: results.interval_type == "day" ? '0' : results.interval_type == "week" ? '1' : '2',
          cycle_base: results.cycle_base,
          remind: results.remind,
          check_method: results.check_type_id,
          is_enable: results.is_open,
          desc: results.desc,
          subject: targetArr,
          trade_label: `${results.trade_abbr}-${results.trade_name}`,
          trade: results.trade_id,
  ...
  handleSubmit(e) {
    ...
    this.props.form.validateFields((err, values) => {
      let {subject, remind, interval, isCycle, cycle_base} = this.state
      if (subject.length < 1) {
      ...
      if (!err) {
        ...
        const data = {
          rule_id: _this.props.location.state.id,
          ...
          cycle_base: values.base ? values.base == 0 ? 'interval' : 'real' : cycle_base,
  ...
  render() {
    ...
    const {
      confirmLoading, spinLoading, isCycle, fetching, data, subject, ques, uid, treeData,
      typelist, name, key_label, related_key, mtype, remind, check_method, desc, interval, unit, fileList, previewImage,
      cycle_base
    } = this.state
    const { formatMessage } = this.props.intl
            ...
            <Form onSubmit={this.handleSubmit}>
              ...
              {/*<FormItem {...formItemLayout} label="类key">*/}
                {/*{getFieldDecorator('trade_key', {*/}
                  {/*initialValue: trade && trade_label ? {key: trade,label: trade_label} : {key: '',label: ''},*/}
                  {/*rules: [*/}
                    {/*{*/}
                      {/*required: true,*/}
                      {/*message: '请选择类',*/}
              ...
              <FormItem {...formItemLayout} label={<FormattedMessage id="page.walkthrough.system_eqpt_key" defaultMessage="系统/设备KEY" />}>
                {getFieldDecorator('related_key', {
                  initialValue: key_label && related_key ? {label: key_label, value: related_key} : {
                    label: null,
                    value: null
                  },
                  rules: [
              ...
              {/*<FormItem {...formItemLayout} label="是否循环:">*/}
                {/*{getFieldDecorator('cycle', {*/}
                  ...
                  {/*],*/}
                {/*})(*/}
                  {/*<Switch onChange={this.onCycleChange}/>*/}
                {/*)}*/}
              {/*</FormItem>*/}
                  ...
                  <Fragment>
                      ...
                      <Row gutter={8}>
                        <Col span={16}>
                          {getFieldDecorator('interval', {
                            initialValue: interval ? interval : null,
                            rules: [
                    ...
                    <FormItem {...formItemLayout} label={<FormattedMessage id="page.walkthrough.base" defaultMessage="循环基于" />} >
                      {getFieldDecorator('base', {
                        initialValue: cycle_base ? cycle_base === 'interval' ? '0' : '1' : null,
                        rules: [
                          {
                            required: true,
              ...
              <FormItem {...formItemLayout} label={<FormattedMessage id="page.walkthrough.common.remind" defaultMessage="提前提醒天数" />}>
                {getFieldDecorator('remind', {
                  initialValue: remind ? remind : null,
                  rules: [
              ...
              <FormItem {...formItemLayout} label={<FormattedMessage id="page.walkthrough.check_type" defaultMessage="检查方式" />}>
                {getFieldDecorator('check_method', {
                  initialValue: check_method ? check_method : null,
                  rules: [
                    {
                      required: true,
...
export default RulesEditForm = Form.create()(RulesEditForm)
