...
export default class extends React.Component {
  ...
  getInfo(params) {
    ...
    RuleSearchEqpt(params).then((res) => {
      ...
      let data = res.data
        ...
        //console.log(result[0].assign_package)
        pagination.total = result[0].assign_data.length
        // console.log(result[0].assign_data.length)
        let temp = result[0].assign_data.map((d,index) => {
          return {...d,...{package: result[0].assign_package[index]}}
        })
        _this.setState({
            data: temp,
            loading: false,
  ...
  exportExcel = () => {
    ...
    _util.exportExcel(selectedRows, column, formatMessage(messages.related))    //已关联
