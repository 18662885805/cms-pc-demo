...
class RulesEditForm extends React.Component {
      ...
      RulesDetail(this.props.location.state.id).then((res) => {
        ...
        const getValue = (obj) => {
          ...
          if (obj.content) {
            tempObj.content = []
            obj.content.map(o => {
              tempObj.content.push({name: o.name,is_fault: o.is_fault == 'True' ? true : false})
            })
          }
          return tempObj
        }
  ...
  render() {
    ...
    const {
      ...
      typelist, name, key_label, related_key, mtype, remind, check_method, desc, interval, unit
    } = this.state
