...
import {CardTypeDetail} from '@apis/facility/cardtype'
...
export default class AllTaskDetail extends React.Component {
  ...
  render() {
    const {name, desc, status, created, created_time, created_name, updated_name, updated_time} = this.state
    ...
    const tableData = [
      ...
      {
        text: <FormattedMessage id="page.walkthrough.maintcard.desc" defaultMessage="描述" />,
        value: _util.getOrNull(desc)
      },
      // {
      //   text: <FormattedMessage id="page.walkthrough.maintcard.status" defaultMessage="状态" />,
