...
import {MyMaintCardPost} from '@apis/facility/mymaintcard'
...
class MaintCardAddForm extends React.Component {
  ...
  componentDidMount() {
    CardType({project_id: _util.getStorage('project_id')}).then((res) => { // 维修类型
      this.setState({
        typelist: res.data.results
      })
    })

    SupplierList({project_id: _util.getStorage('project_id')}).then((res) => { //供应商列表
      this.setState({
  ...
  handleSubmit(e) {
    ...
    this.props.form.validateFields((err, values) => {
      const {number, factory_id, supervisor, supervisor_id, fileList, start_date, end_date, is_supplier} = this.state
  ...
  render() {
    ...
    const {
      confirmLoading, spinLoading, fetching, number, treeData, fileList, previewImage,
      is_supplier, hours, userInfo, typelist, supplierlist
    } = this.state
    ...
    const formItems = keys.map((k, index) => {
      return (
        <div style={{marginTop: '32px'}} key={k}>
          ...
          <FormItem
            {...addItemLayout}
            label={formatMessage(messages.items) + `${index + 2}`}
            required
          >
