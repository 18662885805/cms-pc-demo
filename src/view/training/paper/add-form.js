...
import {papersPost} from '@apis/training/paper'
...
class PapersAddForm extends React.Component {
            ...
            for (let i = 0, len = subject.length; i < len; i++) {
                ...
                if (subject[i].content.length <= 0) {
                    ...
                    if (!content.some(c => c.status)) {
                        message.error(formatMessage(messages.status))       //请设置一项正确选项!
                        return
                    }
    ...
    render() {
        const { survey, length, score, clearance } = this.state
        const { formatMessage } = this.props.intl
                                        ...
                                        return (
                                            <div 
                                                ...
                                                >
                                                    ...
                                                    {
                                                        (survey.subject[qIndex].content.length ? (
                                                            survey.subject[qIndex].content.filter(item => {
                                                                return item.status == 1
                                                            }).length ? null : <span style={{margin:'0',fontSize:'8px'}}><FormattedMessage id="app.papers.check.addQuestion" defaultMessage="请点击单选框设置一项正确选项!" /></span>
                                                        ) : null)
                                                    }
