...
import {trainPut,paperInfo,materialInfo} from '@apis/training/manage'
...
@injectIntl
class TrainingManageAddForm extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
    ...
    if (this.props.location.state === undefined || this.props.location.state.id === undefined) {
        ...
        trainDetail(project_id,{id: this.props.location.state.id}).then((res) => {
            ...
            if(res.data.materials&&res.data.materials.length){
              var initial_materials_list = [];
              res.data.materials.forEach(item => {
                initial_materials_list.push(item.id)
              })
              this.setState({initial_materials_list})
            }
        })
    }
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        confirm({
          ...
          onOk() {
            const project_id = _util.getStorage('project_id');
            trainPut(project_id,values).then((res) => {
            ...
            <Form onSubmit={this.handleSubmit}>
              ...
              <FormItem {...formItemLayout} label={'培训试卷'} >
                {getFieldDecorator('paper', {
                  initialValue:paper_info&&paper_info.id ? paper_info.id :'',
                  rules: [
                    {
                      required: true,
              ...
              <FormItem {...formItemLayout} label={<FormattedMessage id="page.training.material.materialsInfo" defaultMessage="培训资料" />} >
                {getFieldDecorator('materials', {
                  initialValue:initial_materials_list&&initial_materials_list.length?initial_materials_list:[],
                  rules: [
                    {
                      required: true,
                      message: formatMessage(messages.train_materials_check),         
              ...
              <FormItem
                  ...
                  label={'是否是入场培训'}
              >
                 <Checkbox checked={is_entry} onChange={(e) => this.onChangeEntry(e.target.checked)}/>
              </FormItem>
              <FormItem
...
const TrainingManageAdd = Form.create()(TrainingManageAddForm)

export default TrainingManageAdd
