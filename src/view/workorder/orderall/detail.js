...
import {
    allOrderDetail,
} from '@apis/workorder/order-search'
...
const rates = ['非常不满意', '不满意', '一般','满意', '非常满意']

let _util = new CommonUtil()

@inject('menuState') @observer
class OrderSearchDetailForm extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            avatar: '',
    ...
    componentDidMount() {
        ...
        if (this.props.location.state === undefined || this.props.location.state.id === undefined) {
            ...
            allOrderDetail(this.props.location.state.id).then((res) => {
                let data = res.data.results
                this.setState({
                    ...data
                })
                this.props.menuState.changeMenuCurrentUrl('/workorder/orderall')
                this.props.menuState.changeMenuOpenKeys('/workorder')
            })
        }
    }
    ...
    render() {
        ...
        const tableData = [
            {
                text: <FormattedMessage id="page.work.my.title2" defaultMessage="报修单编号"/>,
            ...
            },
            {
                ...
                value: _util.getOrNull(fromuser_phone)
            },
            {
                text: <FormattedMessage id="page.work.my.title5" defaultMessage="报修类型"/>,
                value:  _util.getOrNull(cate_name)
            },
            {
                text:<FormattedMessage id="page.work.my.title6" defaultMessage="优先级"/>,
            ...
            {
                text: <FormattedMessage id="page.work.my.cost1" defaultMessage="花费金额"/>,
                value:cost?cost+'元':'-'
            },
            {
            ...
            // },
            {
                text:<FormattedMessage id="page.work.my.content" defaultMessage="报修内容"/>,
                        ...
                        <Timeline style={{margin: '5px auto -25px'}}>
                            {
                                flowsObj.map((flow, flowIndex) => {
                                    return (
                                        <Timeline.Item key={flowIndex} color={flowIndex===flowsObj.length-1?'grey':'blue'}>
                                            <div
                                                className={''}>
                                                {
                                                    flow.message
        ...
        return (
            <div>
                ...
                <CardDetail title={<FormattedMessage id="page.order.myOrder.orderDetail" defaultMessage="报修详情"/>} data={tableData}  />
                    <GoBackButton 
...
const OrderSearchDetail = Form.create()(OrderSearchDetailForm)

export default injectIntl(OrderSearchDetail)
