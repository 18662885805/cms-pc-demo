...
import {accessCardEnableDetail} from '@apis/security/accesscard'
...
class FactoryapplyAudit extends React.Component {
    ...
    componentDidMount() {
        ...
        if (this.props.location.state === undefined || this.props.location.state.id === undefined) {
            ...
            accessCardEnableDetail(project_id,{id: this.props.location.state.id}).then((res) => {
                this.setState({
                    ...res.data,
                    spinLoading: false,
    ...
    render() {
        const {confirmLoading,staff_info,need_training,status,factory_approve_name,created,updated,
            avatar_url,active,approve_time,remark,is_in,approve,certificateData,cardShow,certificateLoading
        } = this.state
        ...
        const tableData =  [
           ...
           {
                text: '门禁权限',
                value: _util.renderAccessCard(staff_info&&staff_info.access_card)
           },
           {
                text: '当前状态',
                value:_util.getOrNull(is_in ? '已进场': '未进场')
        },
            // {
            //     text: '头像',
        ...
        const bread = [
            ...
            {
                name: '详情',
                url: '/safety/enabled/accesscard/detail'
            }
          ]
