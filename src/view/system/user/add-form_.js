...
import {roleInfo} from "@apis/system/role";
...
class UserAddForm extends React.Component {
  ...
  componentDidMount() {
    ...
    roleInfo({project: _util.getStorage('project_id'), mode: 'all'}).then((res) => {
      const { type } = this.props.location.state
      let role = []
      if(type === 1){
       role = res.data.filter(d => d.name.indexOf('员工') > -1 && d.name === '员工')
       console.log(role)
      }else {
        role = res.data.filter(d => d.name.indexOf('组织') > -1 && d.name === '组织')
      }
      this.setState({
        role,
        roleList: res.data
